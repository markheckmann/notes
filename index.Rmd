---
title: "About the notes"
output:
  html_document:
    toc: true
---

On this site you find some notes I took over the last years on quite diverse topics. Some of the notes are well worked out. Others are unfinished or work in progress and may contain errors. Most notes are in English, some are written in German. Notes which I considered useful for a broader audience are also published on my [R you ready?](http://ryouready.wordpress.com) blog. You can find more info about me on [my personal blog](http://markheckmann.de).

All the source code to produce the notes can be found in one of the folders of the master branch of [this github repository](https://github.com/markheckmann/notes). 

* R
    + [QQ-plots in R vs SPSS](./qq_plot_by_hand.html)
    + [Reguläre Ausdrücke in R](./regular_expressions/regex.pdf)
    + [Funktionen programmieren](./r_funktionen_programmieren/funktionen_programmieren.pdf)
    + [Simulationen mit R (basics)](./simulation_basics/2012_simulation_reworked.pdf)
    + [PNG pictograms in R base plots](./colorize_png_pictograms.html)
    + [Data frames with lists as columns](./dataframes-with-list-as-columns.html)
        
* Stats
    + [Cross Validation](./cross_validation.html)
    + [Regression: Testing custom hypothesis](./regression_custom_hypotheses.html)
    + [Regression: Partielle Regressionskoeffizienten](./regression_partiell.html)
    + [Wallenius' multivariate hypergeometrical distribution](./wallenius-distribution.html)

* Methods 
    + [Thurstone Skalierung](./scaling_thurstone.html)
    + [KTT: Spearman Brown Korrektur](./ctt_spearman_brown.html)

* Linear algebra
    + [SVD: Image compression example](./svd_image_compression.html)

* Papers redone in R
    + [Abdi & Valentin (2007). Multiple factor analysis (MFA)](./multiple_factor_analysis.html)
    + [Tarakci et al. (2014). Strategic consensus mapping](./strategic_consensus_mapping_sray.html)

    
    

